    ...
    def end_turn(self):
        ...
        if self.check_for_endgame():
            if self.turn == CYAN:
                ...
                self.graphics.draw_message("CYAN WINS!")
...
class Board:
    ...
    def new_board(self):
        ...
        for x in xrange(8):
            ...
            for y in xrange(5, 8):
                if matrix[x][y].color == BLACK:
                    matrix[x][y].occupant = Piece(CYAN)
    ...
    def blind_legal_moves(self, (x, y)):
        ...
        if self.matrix[x][y].occupant != None:
            ...
            if self.matrix[x][y].occupant.king == False and self.matrix[x][y].occupant.color == CYAN:
                blind_legal_moves = [
    ...
    def king(self, (x, y)):
        ...
        if self.location((x, y)).occupant != None:
            if (self.location((x, y)).occupant.color == CYAN and y == 0) or (self.location((x, y)).occupant.color == RED and y == 7):
